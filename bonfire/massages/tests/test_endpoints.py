...
class TodaysMassageQueueEntryViewSetTests(APITestCase):
    ...
    def test_downgrade(self):
        ...
        self.assertEqual(self.queue_entry_3.queue_position, 0)
        self.assertEqual(self.queue_entry_2.queue_position, 1)
        self.assertEqual(self.queue_entry_1.queue_position, 2)
        self.assertEqual(self.queue_entry_4.queue_position, 3)

    def test_begin(self):
        response = self.client.patch(self.beign_url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

        self.queue_entry_1.refresh_from_db()

        self.assertEqual(
            self.queue_entry_1.status,
